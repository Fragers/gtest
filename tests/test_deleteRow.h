#ifndef TEST_DELETEROW_H
#define TEST_DELETEROW_H
...
TEST(deleteRow, deleteFirstGroup){
...
//    QApplication a(argc, argv);
    ...
    w.ui->treeView->selectionModel()->setCurrentIndex(ind, QItemSelectionModel::Select);
    w.ui->treeView->closePersistentEditor(w.ui->treeView->selectionModel()->currentIndex());
    std::string path = (const char*)DELETEDIR;

    w.removeRow();

      w.file1.setFileName((path+"deleteTest.json").c_str());
    w.saveClicked();

    QFile getFile;
    getFile.setFileName((path+"deleteTest.json").c_str());
    ...
    QFile haveFile;
    haveFile.setFileName((path+"deleteFirstGroup.json").c_str());
...
//    QApplication a(argc, argv);
    ...
    TreeItem* root = w.model1->getRoot();
    TreeItem* first = root->child(0)->child(1);
    QModelIndex ind = w.model1->getIn(1, 0, first);
    ...
    QFile haveFile;
    haveFile.setFileName((path+"deleteFirstGroupSecondEmployee.json").c_str());
    haveFile.open(QIODevice::ReadOnly);
    QByteArray haveFileAr = haveFile.readAll();
    std::string haveS = haveFileAr.toStdString();
    haveFile.close();
...
//    QApplication a(argc, argv);
    ...
    TreeItem* root = w.model1->getRoot();
    TreeItem* first = root->child(0)->child(1)->child(0);
    QModelIndex ind = w.model1->getIn(0, 0, first);
    ...
    QFile haveFile;
    haveFile.setFileName((path+"deleteTask.json").c_str());
    haveFile.open(QIODevice::ReadOnly);
    QByteArray haveFileAr = haveFile.readAll();
    std::string haveS = haveFileAr.toStdString();
    haveFile.close();
